...
DEBUG_MODE = True  # Установите True для включения отладочной информации
...
async def switch_tabs_and_collect_data(page):
    """
    ...
    try:
        ...
        for i in range(num_tabs):
            try:
                ...
                await page.wait_for_selector("#detail > div.matchHistoryRowWrapper", timeout=2000)

                data, server_info = await extract_data_from_page(page)
...
async def process_match_page(match_url, excel_file_path, browser, semaphore):
    """
    Обрабатывает одну страницу матча
    """
    async with semaphore:
        ...
        try:
            ...
            all_data = await switch_tabs_and_collect_data(page)
            if not all_data:
...
async def process_all_match_links(excel_file_path):
    ...
    async with async_playwright() as p:
        ...
        tasks = [process_match_page(url, excel_file_path, browser, semaphore) for url in match_links]
